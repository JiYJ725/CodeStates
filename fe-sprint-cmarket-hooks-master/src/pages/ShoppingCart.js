...
export default function ShoppingCart({
  ...
  const handleQuantityChange = (quantity, itemId) => {
    const changedItemQuantity = cartItems.map((el) => {
      if (el.itemId === itemId) {
        el.quantity = quantity;
        return el;
      } else return el;
    });
    changeItemQuantityHandler(changedItemQuantity);
  };

  const handleDelete = (itemId) => {
    deleteItemHandler(cartItems.filter((el) => el.itemId !== itemId));
  ...
  //     if (cartItems.map((el) => el.itemId).indexOf(a.id) > -1) {
  //       res.push(a);
  //     }
  //   }
  //   console.log(res);
